...
{	
	...
	// getting total number records without any search
		...
		$journalSql = "SELECT id, date, client, item, qty, type";
		$journalSql.=" FROM journals WHERE 1=1";
	...
	{  //entry_date  
		...
		if(preg_match($pattern_day_month1, $requestData['columns'][2]['search']['value']) || preg_match($pattern_day_month2, $requestData['columns'][2]['search']['value']) || preg_match($pattern_day_month3, $requestData['columns'][2]['search']['value']) || preg_match($pattern_day_month4, $requestData['columns'][2]['search']['value']) || preg_match($pattern_day_month5, $requestData['columns'][2]['search']['value']))
		{
			...
			$journalSql.=" AND date LIKE '%".$month_day."' ";	
		...
		if( preg_match($pattern_month, $requestData['columns'][2]['search']['value']) )
		{
			...
			$journalSql.=" AND month(date) = '".$month."' ";	
		}	
		...
		if(	preg_match($full_pattern, $requestData['columns'][2]['search']['value'])	)
		{
			...
			$journalSql.=" AND date LIKE '".$full_date."' ";	
		}
		...
		else if(strcmp($requestData['columns'][1]['search']['value'] , 'to') != 0)	
		{
			...
			$journalSql.=" AND date BETWEEN '".$from."' AND  '".$to."' ";	
		}	
	...
	{  //ar
		...
		$journalSql.=" AND client LIKE '".$requestData['columns'][5]['search']['value']."%' ";
	}

	if( !empty($requestData['columns'][6]['search']['value']) )
	{
		$sql.=" AND item LIKE '".$requestData['columns'][6]['search']['value']."%' ";
		$journalSql.=" AND item LIKE '".$requestData['columns'][6]['search']['value']."%' ";
	}
	...
	$query=mysqli_query($con, $sql) or die(mysqli_error($con));
	$journalQuery=mysqli_query($con, $journalSql) or die(mysqli_error($con));
	...
	while( $row=mysqli_fetch_array($journalQuery) ) 
	{  // preparing an array
		$nestedData=array(); 
		...
		if($row['type'] == 'slip')
			$nestedData[] = 'Journal Slip';
		else
			$nestedData[] = 'Journal Invoice';
