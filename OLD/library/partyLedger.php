...
// getting total number records without any search
	...
	$str = file_get_contents('searchArray.json');
	$json = json_decode($str, true); // decode the JSON into an associative array
	$client = $json['client'];
	$fromDate = $json['fromDate'];
	$toDate = $json['toDate'];
	
	$sql = "SELECT id, entry_date, client, item, qty, lorry, loading_time, delivery_number, godown_slip_number, invoice_number, godown";
	...
	$sql = "SELECT id, entry_date, client, item, qty, lorry, loading_time, delivery_number, godown_slip_number, invoice_number, godown";
	$sql.=" FROM stock_details WHERE client = '$client' AND entry_date >= '$fromDate' AND entry_date <= 'toDate' ";
...
{ 
	$sql.=" AND delivery_number LIKE '".$requestData['columns'][8]['search']['value']."%' ";
}

if( !empty($requestData['columns'][9]['search']['value']) )
{ 
	$sql.=" AND invoice_number LIKE '".$requestData['columns'][9]['search']['value']."%' ";
}

if( !empty($requestData['columns'][10]['search']['value']) )
{
	$sql.=" AND godown LIKE '".$requestData['columns'][10]['search']['value']."%' ";
}
...
{  // preparing an array
	...
	$nestedData[] = $row["delivery_number"];
