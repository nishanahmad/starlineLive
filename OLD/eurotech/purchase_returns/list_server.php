...
// getting total number records without any search
	...
	$sql = "SELECT id, date";
	$sql.=" FROM purchase_returns ORDER BY date DESC";
	...
	$sql = "SELECT id, date";
	$sql.=" FROM purchase_returns where 1=1  ";
...
{  // preparing an array
	...
	$nestedData[] = '<a href="view.php?id='.$row["id"].'" style="font-size:25px;">pr-'.$row["id"].'</a>';
	$nestedData[] = '<font style="font-size:25px;">'.date('d-m-Y',strtotime($row['date'])).'</font>';
	
	$id = $row['id'];
	$queryItems = mysqli_query($con,"SELECT itemCode,qty FROM purchase_return_items WHERE purchase_return_id ='$id' ");
	
	$count=1;	
	$itemString = '<table class="serverTable"><tr>';
	while( $item=mysqli_fetch_array($queryItems)) 
	{
		//$itemString = $itemString.$item['itemCode'].'<b> : '.$item['qty'].'</b>&emsp;&emsp;';
		$itemString = $itemString.'<td>'.$item['itemCode'].'<b> : '.$item['qty'].'</b></td>';
		if($count % 6 ==0)
		{
			//$itemString = $itemString.'<br/>';
			$itemString = $itemString.'</tr><tr>';
		}	
		$count++;
	}
