...
// getting total number records without any search
	...
	$sql = "SELECT id, date, customerName, customerPhone";
	$sql.=" FROM returns ORDER BY date DESC";
...
{
	$sql.=" AND customerName LIKE '".$requestData['columns'][2]['search']['value']."%' ";
}

if( !empty($requestData['columns'][3]['search']['value']) )
{
	$sql.=" AND customerPhone LIKE '%".$requestData['columns'][3]['search']['value']."%' ";
}
...
while( $row=mysqli_fetch_array($query) ) 
{
	...
	$nestedData[] = '<a href="view.php?id='.$row["id"].'">rtn-'.$row["id"].'</a>';
